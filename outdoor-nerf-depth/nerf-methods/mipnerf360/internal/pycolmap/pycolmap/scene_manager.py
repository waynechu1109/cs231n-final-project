    ...
    def _load_images_txt(self, input_file):
        ...
        with open(input_file, 'r') as f:
            ...
            while line_idx < len(lines):
                ...
                data = line.split()
                image_id = int(data[0])
                image = Image(' '.join(data[9:]), int(data[8]),
                              Quaternion(np.array(list(map(float, data[1:5])))),
                              np.array(list(map(float, data[5:8]))))

                while line_idx < len(lines) and lines[line_idx].lstrip().startswith('#'):
                    line_idx += 1
                ...
                if data:
                    ...
                    image.points2D = np.zeros((0, 2))
                    image.point3D_ids = np.zeros((0,), dtype=np.uint64)
